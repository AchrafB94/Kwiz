...
import { getQuizzesByUser, editQuiz } from "../../redux/actions/quizActions";
            ...
            {this.props.winnersByQuiz.length > 0 ? (
              <table className="table table-borderless">
                <tbody>
                  ...
                  {this.props.winnersByQuiz.map((score) => {
                    return (
                      <tr key={score.id}>
                        <td>
                          <img
              ...
              "Il n'y a pas de médaillés jusqu'à présent."
            )}
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={this.handleClose}>
            ...
            <div className="form-group">
              {this.state.rank === 0 ? (
                ...
                <label htmlFor="rank">
                  ...
                  >
                    {this.props.quizzes
                      .filter((quiz) => quiz.rank > 0)
                      .map((quiz) => {
                        return (
                          <option key={quiz.id} value={quiz.rank}>
                            {quiz.rank}
                          </option>
                        );
                      })}
                        ...
                        <tr key={quiz.id}>
                          <td>
                            <b>
                              <Link
                                to="/contrib/"
                                onClick={this.onClickDetails.bind(
                                  this,
                                  quiz.id
                                )}
                              >
                          ...
                          <td>
                            {quiz.rank === 0 ? (
                              <p>Disponible ({quiz.medals}/3 gagnants)</p>
                            ) : (
                              <p>En attente (position: {quiz.rank})</p>
                            )}
                          </td>
                          <td>{quiz.subject.name}</td>
                          <td>{quiz.level.name}</td>
                          <td>
                            {moment(quiz.createdAt).format("D/MM/Y HH:mm")}
                          </td>
                          <td>
                            {moment(quiz.updatedAt).format("D/MM/Y HH:mm")}
                          </td>
                          <td>
                            <button
                              className="btn-sm btn-primary"
                              onClick={this.onClickModify.bind(this, quiz)}
                            >
                              <FontAwesomeIcon icon="edit" /> Modifier le quiz
                            </button>
                          </td>
                          <td>
                            <Link to={"/contrib/questions/" + quiz.id}>
                              <button className="btn-sm btn-info">
                                <FontAwesomeIcon icon="question-circle" /> Gérer
                                les questions
                              </button>
                            </Link>
                          </td>
                          <td>
                            <button
                              ...
                              onClick={this.onClickMedalists.bind(
                                this,
                                quiz.id
                              )}
                            >
                              <FontAwesomeIcon icon="medal" /> Les médaillées
                            </button>
                          </td>
                          <td>
                            {" "}
