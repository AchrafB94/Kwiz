...
{this.props.scores.map(score =>
  <div key={score.id} className="feed-item">
    ...
    <div className="date">{score.level ? "Niveau "+score.level.name : "Niveau non défini"} - {score.subject ? score.subject.name : "Matiére non défini"} - {score.quiz ? <Link to={"/quiz/"+score.quiz.id} >{score.quiz.name}</Link> : "Quiz non défini" }- {score.createdAt}</div>
