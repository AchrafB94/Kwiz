...
class QuizList extends React.Component {
  ...
  render() {
    if (this.props.link) {
      const quizzesBySubject = this.props.quizzes.filter(
        quiz => quiz.subject.link === this.props.link
      );
      if (this.props.filterId) {
        const filteredQuizzes = quizzesBySubject.filter(
          quiz => quiz.id !== this.props.filterId
        );

        return filteredQuizzes.map(quiz => this.quizCard(quiz));
      } else return quizzesBySubject.map(quiz => this.quizCard(quiz));
    } else
      return (
        <div>
          <h5>
