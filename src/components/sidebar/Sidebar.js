...
class Sidebar extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      return (
        <div key={subject.id} >
          <Link  to={"/subject/" + subject.id} id="subject" className="list-group-item list-group-item-action" >{subject.name}</Link>
          </div>
      )})}
    </div>
    <div className="list-group  list-group-flush" >
    <Link  to={"/forum/"} id="forum" className="list-group-item list-group-item-action" > <FontAwesomeIcon icon="comment-alt" /> 2K-PI</Link>
  </div>
      </div>
    )}else return null
  }
