...
class Settings extends React.Component {
  ...
  checkFileSize = (event) => {
    ...
    if (file.size > size) {
      err +=
        "la taille du " +
        file.name +
        " est trés large, veuillez choisir un fichier plus petit\n";
    }
    if (err !== "") {
  ...
  onSubmitFile(e) {
    ...
    if (this.state.file) {
      ...
      axios.post(`${config.APIEndpoint}/users/upload`, data, {});
  ...
  render() {
    ...
    return (
      <div className="card container mt-5 mb-5">
        <Tabs
          fill
          defaultActiveKey={this.props.match.params.setting}
          ...
          <Tab eventKey="password" title="Mot de passe">
            <div className="container mt-5 mb-5">
              <form onSubmit={this.handlePasswordSubmit}>
                ...
                <Alert
                  dismissible
                  onClose={() => this.setState({ showPasswordAlert: false })}
                  show={this.state.showPasswordAlert}
                  variant="info"
                >
                  <Alert.Heading>
                    <FontAwesomeIcon icon="exclamation-triangle" /> Oups, ces
                ...
                <Alert
                  dismissible
                  onClose={() =>
                    this.setState({ showWrongPasswrodAlert: false })
                  }
                  show={this.state.showWrongPasswrodAlert}
                  variant="danger"
                >
                  <Alert.Heading>
                    <FontAwesomeIcon icon="exclamation-triangle" /> mauvais mot
                    de passe! réessayer.
                  </Alert.Heading>
                  <p>
                    Assurez-vous de taper le mot de passe correct et essayez à
                ...
                <Alert
                  ...
                  onClose={() => this.setState({ showPasswordRules: false })}
                  variant="info"
                >
            ...
            <div className=" container mt-5 mb-5">
              ...
              <form onSubmit={this.onSubmitFile}>
                ...
                <small id="fileHelp" className="form-text text-muted">
                  La taille minimale de l'image doit être: 150 x 150 et son
                  format doit être png, jpeg ou gif.
                </small>
              </form>
            </div>
          </Tab>
